<script>
	...
	function _setSlide(index) {
		slides.update((prev) => ({
			...prev,
			activeSlide: (index >= 0 && index < prev.count)
				? index : prev.activeSlide,
    	}));
	}
    ...
    async function startJoinMeeting(event) {
		...
		slides.update((prev) => ({ ...prev, count: data.length, slideContent: data }));
	}
...
<FirebaseApp {firebase}>
    <User let:user={rawUser} persist={sessionStorage} on:user='{(ev) => (user = ev.detail.user ? parseUser(ev.detail.user) : null)}'>
		<div slot='signed-out'>
			<FirebaseUI />
		</div>
